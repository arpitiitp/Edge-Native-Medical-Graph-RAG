...
class ChunkingConfig:
    ...
    use_semantic_splitting: bool = os.getenv("USE_SEMANTIC_CHUNKING", "true").lower() == "true"
...
class SemanticChunker:
    ...
    def _split_on_structure(self, content: str) -> List[str]:
        ...
        Returns:
            List of sections
        """
        # Split on markdown headers, paragraphs, medical section headers, and other structural elements
        patterns = [
            r'\n#{1,6}\s+.+?\n',  # Markdown headers
            r'\n[A-Z\s]+:\s*\n?', # Medical report headers (e.g., "CHIEF COMPLAINT:")
    ...
    async def _split_long_section(self, section: str) -> List[str]:
        ...
        try:
            prompt = f"""
            Split the following medical or administrative text into semantically coherent clinical chunks. Each chunk should:
            1. Be roughly {self.config.chunk_size} characters long
            2. End at natural semantic boundaries (e.g., end of a clinical observation, distinct policy section)
            3. Maintain medical context, keeping related symptoms, diagnoses, or policy rules together
