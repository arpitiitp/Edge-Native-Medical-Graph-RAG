...
class EmbeddingGenerator:
    ...
    def __init__(
        ...
        if model not in self.model_configs:
            logger.warning(f"Unknown model {model}, using default config from env")
            self.config = {"dimensions": vector_dim, "max_tokens": 8192}
        else:
            self.config = self.model_configs[model]
            # Always respect the environment variable if provided
            if os.getenv("VECTOR_DIMENSION"):
                self.config["dimensions"] = vector_dim
